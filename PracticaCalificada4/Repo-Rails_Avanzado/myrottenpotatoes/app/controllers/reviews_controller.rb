class ReviewsController < ApplicationController
    ...
    def edit
        # Solo permite editar si el usuario es el autor de la crítica
        authorize_user(@review.user)
      end
    
      def update
        # Solo permite actualizar si el usuario es el autor de la crítica
        authorize_user(@review.user)
    
        if @review.update(review_params)
          redirect_to movie_path(@movie), notice: "Critica actualizada"
        else
          flash[:alert] = "No puede ser actualizada " +
